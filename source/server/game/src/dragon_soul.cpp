...
bool DSManager::RefreshItemAttributes(LPITEM pDS)
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		int sValue = (int)(ceil((float)basic_apply.apply_value * fWeight - 0.01f));
	...
	for (int i = DRAGON_SOUL_ADDITIONAL_ATTR_START_IDX; i < ITEM_ATTRIBUTE_MAX_NUM; i++)
	{
		...
		pDS->SetForceAttribute(i, bType, (int)(ceil((float)sValue * fWeight - 0.01f)));
	}
	return true;
}
...
bool DSManager::PutAttributes(LPITEM pDS)
{
	...
	if (additional_attr_num > 0)
	{
		...
		for (int i = 0; i < additional_attr_num; i++)
		{
			...
			int sValue = (int)(ceil((float)additional_attr.apply_value * fWeight - 0.01f));
