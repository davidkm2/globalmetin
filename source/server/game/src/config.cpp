...
std::string makine_ip = "";
#endif

#ifdef __INSTANT_ONLINE__
short g_InstantOnline = 0;
#endif
...
std::string g_table_postfix = "";

std::string g_stQuestDir = "./quest";
//std::string g_stQuestObjectDir = "./quest/object";
std::string g_stDefaultQuestObjectDir = "./quest/object";
...
std::string g_stBlockDate = "30000705";
...
int g_server_id = 0;
std::string g_strWebMallURL = "www.metin2.de";
...
#ifdef __LISANS_SYSTEM__
		makine_ip = inet_ntoa(sai->sin_addr);
#endif
...
#ifdef __INSTANT_ONLINE__
		TOKEN("instant_online")
		{
			str_to_number(g_InstantOnline, value_string);
			continue;
		}
#endif
...
static bool __LoadGeneralConfigFile(const char* configName)
{
	...
	while (fgets(buf, 256, fp))
	{
		...
		TOKEN("basic_item")
		{
			str_to_number(g_bBasicItem, value_string);
			continue;
		}
		TOKEN("shutdowned")
		{
...
#ifdef __EXPTABLE_FROMDB__
static bool __LoadExpTableFromDB(void)
{
	std::unique_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT level, exp FROM exp_table"));
...
{
	// LOCALE_SERVICE
	std::string	st_configFileName = "CONFIG";
