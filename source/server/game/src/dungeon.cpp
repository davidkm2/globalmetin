...
namespace
{
	struct FNotice
	{
		FNotice(const char * psz, const bool bBigFont) : m_psz(psz), m_bBigFont(bBigFont)
		{
		}
		...
		void operator() (LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				BYTE type = CHAT_TYPE_NOTICE;
				if (m_bBigFont)
					type = CHAT_TYPE_BIG_NOTICE;

				LPCHARACTER ch = (LPCHARACTER) ent;
				ch->ChatPacket(type, "%s", m_psz);
			}
		}

		const char * m_psz;
		const bool m_bBigFont;
	};
}

void CDungeon::Notice(const char* msg, bool bBigFont)
{
...
EVENTINFO(dungeon_cannon_info)
{
	...
	dungeon_cannon_info() : boss(NULL), cannon(NULL), is_cannon_dead(false), pDungeon(NULL)
	{
