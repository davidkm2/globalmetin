...
namespace quest
{
	...
	int dungeon_notice(lua_State* L)
	{
		...
		bool bBigNotice = false;
		bBigNotice = lua_toboolean(L, 2);

		if (pDungeon)
			pDungeon->Notice(lua_tostring(L, 1), bBigNotice);
		return 0;
	}
...
#endif
#ifdef __12ZI__
				if (ch->IsRaceFlag(RACE_FLAG_ATT_CZ) && !ch->IsStone())
				{
	...
	int dungeon_start_cannon_attack(lua_State* L)
	{
		CQuestManager& q = CQuestManager::instance();
		LPDUNGEON pDungeon = q.GetCurrentDungeon();
