...
namespace marriage
{
	...
	void TMarriage::Login(LPCHARACTER ch)
	{
		...
		if (is_married)
		{
			...
			if (d1 && d2)
			{
				if (d1->GetCharacter() && d2->GetCharacter())
				{
					d1->GetCharacter()->ChatPacket(CHAT_TYPE_COMMAND, "lover_login");
					d2->GetCharacter()->ChatPacket(CHAT_TYPE_COMMAND, "lover_login");
				}
	...
	void TMarriage::Logout(DWORD pid)
	{
		...
		if (is_married)
		{
			...
			if (d1 && !g_bShutdown && d1->GetCharacter())
			{
				d1->GetCharacter()->ChatPacket(CHAT_TYPE_COMMAND, "lover_logout");
			}
			...
			if (d2 && !g_bShutdown && d2->GetCharacter())
			{
				d2->GetCharacter()->ChatPacket(CHAT_TYPE_COMMAND, "lover_logout");
			}
		}
	}
