...
CGuildMarkImage::CGuildMarkImage(): m_uImg (INVALID_HANDLE)
{
...
bool CGuildMarkImage::SaveBlockFromCompressedData(DWORD posBlock, const BYTE * pbComp, DWORD dwCompSize)
{
	...
	if (LZO_E_OK != lzo1x_decompress_safe(pbComp, dwCompSize, (BYTE *) apxBuf, &sizeBuf, LZOManager::Instance().GetWorkMemory()))
	{
...
void SGuildMarkBlock::Compress(const Pixel* pxBuf)
{
	...
	if (LZO_E_OK != lzo1x_1_compress((const BYTE *) pxBuf, sizeof(Pixel) * SGuildMarkBlock::SIZE, m_abCompBuf, &m_sizeCompBuf, LZOManager::Instance().GetWorkMemory()))
	{
