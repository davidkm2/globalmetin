...
ACMD(do_item)
{
	...
	if (item)
	{
		...
		{
			int iEmptyPos = ch->GetEmptyStorageInventory(item->GetStorageType() - UPGRADE_INVENTORY, item);
...
ACMD(do_item_purge)
{
	...
	for (int j = 0; j < STORAGE_INVENTORY_COUNT; ++j)
		for (i = 0; i < STORAGE_INVENTORY_MAX_NUM; ++i)
			if ((item = ch->GetItem(TItemPos(UPGRADE_INVENTORY + j, i ))))
				ITEM_MANAGER::instance().RemoveItem(item, "PURGE");
#endif
...
ACMD(do_event_flag)
{
	...
	if (!strcmp(arg1, "__PARTY_MATCH___giris"))
	{
		if (value != 0)
...
#ifdef __SET_STATE_WITH_TARGET__
			" [<character name>]"
#endif
		);
		return;
...
{
	CBattleField::instance().CloseEnter(true);
}
