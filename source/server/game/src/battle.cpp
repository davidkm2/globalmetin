...
int CalcAttBonus(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, int iAtk)
{
	...
	{
		if (pkAttacker)
			pkAttacker->ChatPacket(0, "Bas:(%d) %d %d", (pkVictim->GetMobTable().dwRaceFlag & RACE_FLAG_ATT_FIRE), pkVictim->IsRaceFlag(RACE_FLAG_ATT_FIRE), iAtk);
		...
		{
			// if (pkAttacker)
			pkAttacker->ChatPacket(0, "Alev att bas:%d", iAtk);
			iAtk += (iAtk * pkAttacker->GetPoint(POINT_ENCHANT_FIRE)) / 100;
			// if (pkAttacker)
			pkAttacker->ChatPacket(0, "Alev att son:%d", iAtk);
		}
...
int battle_hit(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, int & iRetDam)
{
	...
	if (pkAttacker && pkVictim && pkAttacker->GetDesc())
	{
		...
		if (pkVictim->GetRaceNum() >= 6415 && pkVictim->GetRaceNum() <= 6419)
		{
			...
			{
				for (DWORD i = 0; i < pkAttacker->m_Accumulate.size(); i++)
				{
