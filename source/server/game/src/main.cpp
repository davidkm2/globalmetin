...
#include "../../liblisans/include/lisans.h"
#endif
...
void heartbeat(LPHEART ht, int pulse)
{
	...
	CShopManager::instance().ProcessOfflinePrivateShop();
#endif
...
#endif
#ifdef __PARTY_MATCH__
	CGroupMatchManager GroupMatchManager;
#endif
#ifdef __GUILD_DRAGONLAIR__
	MeleyLair::CMgr	MeleyLairManager;
#endif
#ifdef __DAMAGE_TOP__
	CDamageTop DamageTop;
#endif
#ifdef __BATTLE_FIELD__
	CBattleField BattleField;
#endif
#ifdef __RANKING__
	CRankingManager	RankingManager;
#endif
#ifdef __MINI_GAME__
	CMiniGameManager MiniGameManager;
#endif
#ifdef __MAILBOX__
	CMailBoxManager MailBoxManager;
#endif
...
#endif
#ifdef __PARTY_MATCH__
	GroupMatchManager.Initialize();
#endif
#ifdef __DAMAGE_TOP__
	CDamageTop::instance().Initialize();
#endif
#ifdef __RANKING__
...
#endif
#ifdef __GROWTH_PET__
	char exppettableFileName[256];
	snprintf(exppettableFileName, sizeof(exppettableFileName), "%s/exppettable.txt", LocaleService_GetBasePath().c_str());

    std::string temp_exp_line;
	std::ifstream exppet_table_open(exppettableFileName);

	if (!exppet_table_open.is_open())
	{
		sys_err("exppettable.txt couldn't open");
		return 0;
	}
...
#ifndef _WIN32
	// Redirect stdout to file if not on windows
	freopen("stdout", "a", stdout);
#endif

	bool is_thecore_initialized = thecore_set(PASSES_PER_SEC(1), heartbeat); // default: 25
...
#ifndef __LISANS_SYSTEM__
	int asd[4];
	getip(asd);
	if (Setup(makine_ip, asd))
		fprintf(stderr, "IP LISANS GECERLI IP: %s\n", makine_ip.c_str());
	else
	{
		fprintf(stderr, "IP LISANS GECERSIZ IP: %s\n", makine_ip.c_str());
		return 0;
	}
#endif
