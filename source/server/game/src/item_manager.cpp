...
LPITEM ITEM_MANAGER::CreateItem(DWORD vnum, DWORD count, DWORD id, bool bTryMagic, int iRarePct, bool bSkipSave)
{
	...
	TItemTable* table = GetTable(vnum);
#else
	...
	switch(table->bType)
	{
		case ITEM_COSTUME:
		{
			switch(table->bSubType)
			{
				...
				case COSTUME_EFFECT:
				{
					...
					{
						if (table->aLimits[i].bType == LIMIT_REAL_TIME_START_FIRST_USE || table->dwVnum == 71241 || (table->dwVnum >= 73001 && table->dwVnum <= 73762) || (table->dwVnum >= 75621 && table->dwVnum <= 75632))
							break;
						else if (table->aLimits[i].bType == LIMIT_REAL_TIME)
						{
							table->aLimits[i].lValue = 259200;
							break;
						}
						else if (table->aLimits[i].bType == LIMIT_NONE)
						{
							table->aLimits[i].bType = LIMIT_REAL_TIME;
							table->aLimits[i].lValue = 259200;
							break;
						}
						else
		...
		case ITEM_PET:
		{
			if (table->bSubType == PET_PAY)
			{
				for (int i = 0 ; i < ITEM_LIMIT_MAX_NUM ; i++)
				{
					if (table->aLimits[i].bType == LIMIT_REAL_TIME)
					{
						table->aLimits[i].lValue = 259200;
						break;
					}
...
void ITEM_MANAGER::SaveSingleItem(LPITEM item)
{
	...
	memcpy(t.prices, item->GetOfflinePrivateShopPrices(), sizeof(t.prices));
#endif
