...
void SendAffectRemovePacket(LPDESC d, DWORD pid, CAffect * pkAff)
{
	TPacketGCAffectRemove ptoc;
	ptoc.bHeader	= HEADER_GC_AFFECT_REMOVE;
	ptoc.dwType		= pkAff->dwType;
	ptoc.bApplyOn	= pkAff->bApplyOn;
#ifdef __EXPRESSING_EMOTION__
	if (pkAff->dwType == AFFECT_EMOTION)
		ptoc.bApplyOn = pkAff->lApplyValue;
#endif
	d->Packet(&ptoc, sizeof(TPacketGCAffectRemove));

	TPacketGDRemoveAffect ptod;
	ptod.dwPID		= pid;
	ptod.dwType		= pkAff->dwType;
	ptod.bApplyOn	= pkAff->bApplyOn;
...
#ifdef __EXPRESSING_EMOTION__
CAffect * CHARACTER::FindEmotion(DWORD dwType, BYTE bApply, long lApplyValue) const
{
	itertype(m_list_pkAffect) it = m_list_pkAffect.begin();
	...
	while (it != m_list_pkAffect.end())
	{
		...
		if (pkAffect->dwType == dwType && (bApply == APPLY_NONE || bApply == pkAffect->bApplyOn) && lApplyValue == pkAffect->lApplyValue)
			return pkAffect;
	}
...
BYTE CHARACTER::GetEmotionCount() const
{
	itertype(m_list_pkAffect) it = m_list_pkAffect.begin();
	...
	while (it != m_list_pkAffect.end())
	{
		...
		if (pkAffect->dwType == AFFECT_EMOTION)
			bCount++;
	}
...
void CHARACTER::ClearAffect(bool bSave)
{
	...
	while (it != m_list_pkAffect.end())
	{
		...
		if (bSave)
		{
			...
			if (IsPC())
			{
				SendAffectRemovePacket(GetDesc(), GetPlayerID(), pkAff);
			}
		}
...
#ifdef __EXPRESSING_EMOTION__
	else if (dwType == AFFECT_EMOTION)
		pkAff = FindEmotion(dwType, bApplyOn, lApplyValue);
#endif
	else
