...
struct FPartyDropDiceRoll
{
	...
	void Process(const LPCHARACTER mobVictim)
	{
		if ((!mobVictim || (mobVictim->GetMobRank() >= MOB_RANK_BOSS && mobVictim->GetMobRank() <= MOB_RANK_KING)) && m_itemOwner->GetParty() && m_itemOwner->GetParty()->GetNearMemberCount() > 1)
		{
			...
			pParty->ChatPacketToAllMember(CHAT_TYPE_DICE_INFO, "|cffffe6ba{{1201, %s}}", m_itemDrop->GetName());

			pParty->ForEachNearMember(*this);
			if (m_itemOwner)
			{
				m_itemDrop->SetOwnership(m_itemOwner);
				pParty->ChatPacketToAllMember(CHAT_TYPE_DICE_INFO, "|cffbb00ff{{1203, %s, %s}}", m_itemDrop->GetName(), m_itemOwner->GetName());
			}
		}
		else
			m_itemDrop->SetOwnership(m_itemOwner);
	...
	void operator () (LPCHARACTER ch)
	{
		...
		while (true)
		{
			...
			pParty->ChatPacketToAllMember(CHAT_TYPE_DICE_INFO, "|cffffe6ba{{1202, %s, %d}}", ch->GetName(), pickedNumber);
			break;
		}
	}
};
