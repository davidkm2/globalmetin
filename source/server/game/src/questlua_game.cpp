...
namespace quest
{
	...
	int battle_field_get_open_time_list(lua_State* L)
	{
		CBattleField::instance().GetOpenTimeList(L);
		return 1;
	}
#endif

#ifdef __12ZI__
	int game_open_reward(lua_State* L)
	{
		CQuestManager& q = CQuestManager::instance();
		LPCHARACTER ch = q.GetCurrentCharacterPtr();
	...
	int game_open_zodiac_minimap(lua_State* L)
	{
		CQuestManager& q = CQuestManager::instance();
		LPCHARACTER ch = q.GetCurrentCharacterPtr();
...
#ifdef __RANKING__
	int game_open_zodiac_ranking(lua_State* L)
	{
		CQuestManager& q = CQuestManager::instance();
		LPCHARACTER ch = q.GetCurrentCharacterPtr();
		int portal = (int) lua_tonumber(L, 1);
	...
	int game_add_zodiac_ranking(lua_State* L)
	{
		TRankingInfo Info;

		Info.bCategory = (int)lua_tonumber(L, 1) + 2 + SOLO_RK_CATEGORY_MAX;
...
#ifdef __BATTLE_FIELD__
			{ "battle_field_get_open_time_list",		battle_field_get_open_time_list			},
#endif
#ifdef __12ZI__
