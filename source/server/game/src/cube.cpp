...
#endif
{
	...
	if ((inven_type == INVENTORY && ch->GetInventoryMax() <= inven_index) || 
		(inven_type == UPGRADE_INVENTORY && STORAGE_INVENTORY_MAX_NUM <= inven_index) ||
		(inven_type == BOOK_INVENTORY && STORAGE_INVENTORY_MAX_NUM <= inven_index) ||
		(inven_type == STONE_INVENTORY && STORAGE_INVENTORY_MAX_NUM <= inven_index) ||
		(inven_type == ATTR_INVENTORY && STORAGE_INVENTORY_MAX_NUM <= inven_index) ||
		(inven_type == GIFTBOX_INVENTORY && STORAGE_INVENTORY_MAX_NUM <= inven_index))
		return;

	if (inven_type != INVENTORY && inven_type != UPGRADE_INVENTORY && inven_type != BOOK_INVENTORY && inven_type != STONE_INVENTORY && inven_type != ATTR_INVENTORY && inven_type != GIFTBOX_INVENTORY)
		return;
#else
	if (inven_index < 0 || ch->GetInventoryMax() <= inven_index)
		return;
#endif
