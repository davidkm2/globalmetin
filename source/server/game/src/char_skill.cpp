	...
	else
	{
...

		{
			...
			if (pPC)
			{
				...
				{
					// 책읽기에 성공
#ifdef __MASTER_SKILLBOOK_NO_STEPS__
					if (true)
#else
...
#if defined(__WOLFMAN__) && defined(USE_ITEM_CLAW_AS_DAGGER)
								iDam = iDam * (100 - pkChrVictim->GetPoint(POINT_RESIST_DAGGER)) / 100;
#endif
								break;
#endif
...
#if defined(__WOLFMAN__) || defined(__PARTY_FLAG__)
...
int CHARACTER::ComputeSkill(DWORD dwVnum, LPCHARACTER pkVictim, BYTE bSkillLevel)
{
	...
	if (dwVnum != SKILL_MUYEONG && false == bCanUseHorseSkill && true == IsRiding())
		return BATTLE_NONE;
	...
	if (dwVnum != SKILL_MUYEONG && bCanUseHorseSkill && pkSk->dwType != SKILL_TYPE_HORSE)
		return BATTLE_NONE;
...
// #ifdef __12ZI__
	// if (IsMonster() && IsRaceFlag(RACE_FLAG_ATT_CZ))
	// {
		// EffectPacket(SE_EFFECT_SHELTER);
	// }
// #endif
...
#if defined(__WOLFMAN__) && !defined(USE_WOLFMAN_BOOKS)
