						...
						for (int i = 0; i < count; i++){
							switch (dwVnums[i])
							{
								...
								default:
									...
									{
										if (dwCounts[i] > 1)
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("»óÀÚ¿¡¼­ %s °¡ %d °³ ³ª¿Ô½À´Ï´Ù."), item_gets[i]->GetMultiName(), dwCounts[i]);
										else
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("»óÀÚ¿¡¼­ %s °¡ ³ª¿Ô½À´Ï´Ù."), item_gets[i]->GetMultiName());
					...
					case USE_BAIT:
						{
							...
							{
								ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ÀÌ¹Ì ²ÈÇôÀÖ´ø ¹Ì³¢¸¦ »©°í %s¸¦ ³¢¿ó´Ï´Ù."), item->GetMultiName());
							}
							else
							{
								ChatPacket(CHAT_TYPE_INFO, LC_TEXT("³¬½Ã´ë¿¡ %s¸¦ ¹Ì³¢·Î ³¢¿ó´Ï´Ù."), item->GetMultiName());
							}
					...
					case USE_RECIPE :
						{
							...
							if (pSource1 != NULL)
							{
								...
								{
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Àç·á(%s)°¡ ºÎÁ·ÇÕ´Ï´Ù."), pSource1->GetMultiName());
									return false;
								}
							...
							if (pSource2 != NULL)
							{
								...
								{
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Àç·á(%s)°¡ ºÎÁ·ÇÕ´Ï´Ù."), pSource2->GetMultiName());
									return false;
								}
...
bool CHARACTER::MoveItem(TItemPos Cell, TItemPos DestCell, BYTE count)
{
	...
	else
	{
		...
		if ((item2 = GetItem(DestCell)) && item != item2 && item2->IsStackable() && item2->GetVnum() == item->GetVnum()) // ÇÕÄ¥ ¼ö ÀÖ´Â ¾ÆÀÌÅÛÀÇ °æ¿ì
		{
			for (int i = 0; i < ITEM_SOCKET_MAX_NUM; ++i)
	...
	if (item->DistanceValid(this))
	{
		...
		if (item->IsOwnership(this))
		{
			...
			else
			{
				...
				if ((item->IsStorageItem()) && item->IsStackable())
				{
					...
					for (int i = 0; i < STORAGE_INVENTORY_MAX_NUM; ++i)
					{
						...
						if ((item2->GetVnum() == item->GetVnum() && !item->IsBook()) || (item->IsBook() && item2->GetVnum() == item->GetVnum() && item2->GetSocket(0) == item->GetSocket(0)))
						{
							...
							if (bCount == 0)
							{
								ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¾ÆÀÌÅÛ È¹µæ: %s"), item2->GetMultiName());
								M2_DESTROY_ITEM(item);
								return true;
							}
						}
				...
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¾ÆÀÌÅÛ È¹µæ: %s"), item->GetMultiName());
		...
		else if (!IS_SET(item->GetAntiFlag(), ITEM_ANTIFLAG_GIVE | ITEM_ANTIFLAG_DROP) && GetParty())
		{
			...
			{
				owner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¾ÆÀÌÅÛ È¹µæ: %s ´ÔÀ¸·ÎºÎÅÍ %s"), GetName(), item->GetMultiName());
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¾ÆÀÌÅÛ Àü´Þ: %s ´Ô¿¡°Ô %s"), owner->GetName(), item->GetMultiName());
			}
...
bool CHARACTER::ItemProcess_Polymorph(LPITEM item)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ë‘”ê°‘ì—ž ì‹¤íŒ¨ í•˜ì˜€ìŠµë‹ˆë‹¤"));
		return false;
	}
