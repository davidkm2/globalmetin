...
#ifdef __WOLFMAN__
#ifdef __USE_WOLFMAN_CREATE_POSITION__
...
static void _send_bonus_info(LPCHARACTER ch)
{
	...
	if (item_drop_bonus)
	{
		ch->ChatPacket(CHAT_TYPE_NOTICE,
				LC_TEXT("ÏïÑÏù¥ÌÖú ÎìúÎ°≠Î•†  %d%% Ï∂îÍ∞Ä Ïù¥Î≤§Ìä∏ Ï§ëÏûÖÎãàÎã§."), item_drop_bonus);
	}
	if (gold_drop_bonus)
	{
		ch->ChatPacket(CHAT_TYPE_NOTICE,
				LC_TEXT("Í≥®Îìú ÎìúÎ°≠Î•† %d%% Ï∂îÍ∞Ä Ïù¥Î≤§Ìä∏ Ï§ëÏûÖÎãàÎã§."), gold_drop_bonus);
	}
	if (gold10_drop_bonus)
	{
		ch->ChatPacket(CHAT_TYPE_NOTICE,
				LC_TEXT("ÎåÄÎ∞ïÍ≥®Îìú ÎìúÎ°≠Î•† %d%% Ï∂îÍ∞Ä Ïù¥Î≤§Ìä∏ Ï§ëÏûÖÎãàÎã§."), gold10_drop_bonus);
	}
	if (exp_bonus)
	{
		ch->ChatPacket(CHAT_TYPE_NOTICE,
				LC_TEXT("Í≤ΩÌóòÏπò %d%% Ï∂îÍ∞Ä ÌöçÎìù Ïù¥Î≤§Ìä∏ Ï§ëÏûÖÎãàÎã§."), exp_bonus);
	}
}
...
#if defined(__WOLFMAN__) && defined(__USE_WOLFMAN_CREATE_POSITION__)
...
#ifdef __ANTI_LONG_NAME_CREATE__
...
void CInputLogin::Entergame(LPDESC d, const char * data)
{
...
#ifdef __MAILBOX__
	CMailBoxManager::instance().CheckMails(ch);
#endif
...
#ifdef __12ZI__
	ch->SetQuestFlag("zodiac.Insignia1", 0);
	ch->SetQuestFlag("zodiac.Insignia2", 0);
	ch->SetQuestFlag("zodiac.Insignia3", 0);
	ch->SetQuestFlag("zodiac.Insignia4", 0);
	ch->SetQuestFlag("zodiac.Insignia5", 0);
	ch->SetQuestFlag("zodiac.Insignia6", 0);
	ch->SetQuestFlag("zodiac.Insignia7", 0);
	ch->SetQuestFlag("zodiac.Insignia8", 0);
	ch->SetQuestFlag("zodiac.Insignia9", 0);
	ch->SetQuestFlag("zodiac.Insignia10", 0);
	ch->SetQuestFlag("zodiac.Insignia11", 0);
	ch->SetQuestFlag("zodiac.Insignia12", 0);
	ch->SetQuestFlag("zodiac.Insignia13", 0);
	ch->SetQuestFlag("zodiac.Insignia14", 0);
	ch->SetQuestFlag("zodiac.Insignia15", 0);
	ch->SetQuestFlag("zodiac.Insignia16", 0);
	ch->SetQuestFlag("zodiac.Insignia17", 0);
	ch->SetQuestFlag("zodiac.Insignia18", 0);
	ch->SetQuestFlag("zodiac.Insignia19", 0);
	ch->SetQuestFlag("zodiac.Insignia20", 0);
	ch->SetQuestFlag("zodiac.Insignia21", 0);
	ch->SetQuestFlag("zodiac.Insignia22", 0);
	ch->SetQuestFlag("zodiac.Insignia23", 0);
	ch->SetQuestFlag("zodiac.Insignia24", 0);
	...
	{
		DWORD dwTime = time(NULL) - ch->GetLastPlay();
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "IN: %d %d %d %d", ch->GetBeadTime(), ch->GetBead(), ch->GetLastPlay(), time(NULL) - ch->GetLastPlay());
#endif
	...
	if (g_bCheckClientVersion)
	{
		...
		else
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("ÌÅ¥ÎùºÏù¥Ïñ∏Ìä∏ Î≤ÑÏ†ÑÏù¥ ÌãÄÎ†§ Î°úÍ∑∏ÏïÑÏõÉ Îê©ÎãàÎã§. Ï†ïÏÉÅÏ†ÅÏúºÎ°ú Ìå®Ïπò ÌõÑ Ï†ëÏÜçÌïòÏÑ∏Ïöî."));
	...
	if (g_noticeBattleZone)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("Ïù¥ ÎßµÏóêÏÑ† Í∞ïÏ†úÏ†ÅÏù∏ ÎåÄÏ†ÑÏù¥ ÏûàÏùÑÏàò ÎèÑ ÏûàÏäµÎãàÎã§."));
			ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("Ïù¥ Ï°∞Ìï≠Ïóê ÎèôÏùòÌïòÏßÄ ÏïäÏùÑÏãú"));
			ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("Î≥∏Ïù∏Ïùò Ï£ºÏÑ± Î∞è Î∂ÄÏÑ±ÏúºÎ°ú ÎèåÏïÑÍ∞ÄÏãúÍ∏∞ Î∞îÎûçÎãàÎã§."));
		}
	}
}
...
int CInputLogin::GuildSymbolUpload(LPDESC d, const char* c_pData, size_t uiBytes)
{
	...
	{
		// 64k ®¨¢¨¢•U A°Ï °æ©°•Ïa ®ˆE®¨®˘A®¨ ¢ØA¢¨°æ®˘o®˙©™¢•U
		// A°À®˘OA°Ì ©˜°¿°∆i ©ˆ°Ï®ˆA
		d->SetPhase(PHASE_CLOSE);
		return 0;
	}

	// ¢“°ÕA°Ì ®˘OA?CIAo ®˙EA®¨ °æ©°•ÏaAI °∆©°¢Øi.
...
void CInputLogin::GuildMarkCRCList(LPDESC d, const char* c_pData)
{
	...
	DWORD blockCount = 0;
	TEMP_BUFFER buf(1024 * 1024); // 1M ©ˆo®°U
