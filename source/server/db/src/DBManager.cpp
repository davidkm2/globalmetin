...
int CDBManager::Connect(int iSlot, const char * db_address, const int db_port, const char * db_name, const char * user, const char * pwd)
{
	...
	if (!m_directSQL[iSlot]->Setup(db_address, user, pwd, db_name, "latin5", true, db_port))
	{
		Clear();
		return false;
	}
	...
	if (!m_mainSQL[iSlot]->Setup(db_address, user, pwd, db_name, "latin5", false, db_port))
	{
		Clear();
		return false;
	}
	...
	if (!m_asyncSQL[iSlot]->Setup(db_address, user, pwd, db_name, "latin5", false, db_port))
	{
		Clear();
		return false;
	}
