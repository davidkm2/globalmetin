...
{
	for (auto it_cont : m_WarMap)
	{
		for (auto it : it_cont.second)
		{
...
void CGuildManager::BootReserveWar()
{
	...
	for (int i = 0; i < 2; ++i)
	{
		...
		while ((row = mysql_fetch_row(pmsg->Get()->pSQLResult)))
		{
			...
			if (!i || (int) t.dwTime - CClientManager::instance().GetCurrentTime() < 0)
			{
				if (!i)
...
bool CGuildManager::ReserveWar(TPacketGuildWar * p)
{
	...
	if (!pmsg->Get()->uiAffectedRows || !pmsg->Get()->uiInsertID || pmsg->Get()->uiAffectedRows == (uint32_t)-1)
	{
