...
namespace marriage
{
	...
	void CManager::Add(DWORD dwPID1, DWORD dwPID2, const char* szName1, const char* szName2)
	{
		...
		if (!res->uiAffectedRows || res->uiAffectedRows == (uint32_t)-1)
		{
	...
	void CManager::OnSetup(CPeer* peer)
	{
		for (itertype(m_Marriages) it = m_Marriages.begin(); it != m_Marriages.end(); ++it)
		{
			...
			TPacketMarriageUpdate p2;
			p2.dwPID1 = pMarriage->pid1;
			p2.dwPID2 = pMarriage->pid2;
			p2.iLovePoint = pMarriage->love_point;
			p2.byMarried	= pMarriage->is_married;
			peer->EncodeHeader(HEADER_DG_MARRIAGE_UPDATE, 0, sizeof(p2));
