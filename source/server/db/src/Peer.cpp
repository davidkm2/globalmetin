...
{
	if (!m_itemSpareRange.dwMin || !m_itemSpareRange.dwMax || !m_itemSpareRange.dwUsableItemIDMin)
	{
	...
	else
	{
		...
		if (!SetSpareItemIDRange(CItemIDRangeManager::instance().GetRange()))
		{
...
{
	if (!itemRange.dwMin || !itemRange.dwMax || !itemRange.dwUsableItemIDMin)
		return false;
