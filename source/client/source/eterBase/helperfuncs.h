...
namespace msl
{
	...
	template <class T = std::vector <std::string>> T 
		string_split(const std::string & str, char tok = ' ')
	{
		T vec;
		std::size_t prev = 0;
	...
	template <class T = std::vector <std::string>> T 
		string_split(const std::string & str, const std::string & tok = " ")
	{
		T vec;
		std::size_t prev = 0;
		auto cur = str.find(tok);
	...
	template <class T = std::vector <std::string>> T 
		string_split_any(const std::string & str, const std::string & toks = " ")
	{
		T vec;
		std::size_t prev = 0;
	...
	std::string string_join(const T & vec, const char tok = ' ')
	{
		std::string str;
		for (auto & elem : vec)
		{
	...
	constexpr void refill(_Ty(&_Array)[_Size]) noexcept
	{
	...
	template <class _Ty, std::size_t _Size> 
	constexpr void refill(_Ty(&_Array)[_Size], _Ty _Elem) noexcept
	{
	...
	constexpr void refill(_Container & _Cont)
	{
	...
	constexpr void refill(_Container & _Cont, _Ty _Elem)
	{
