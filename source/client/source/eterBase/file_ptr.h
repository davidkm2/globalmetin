...
namespace msl
{
	...
	public:
		...
		explicit file_ptr(const std::string & fn, const char * mode = "r") : 
			file_ptr(fn.c_str(), mode) 
		{
		};

		explicit file_ptr(const char * filename, const char * mode = "r")
		{
			open(filename, mode);
		}

		explicit file_ptr(std::FILE * ptr)
		{
			m_ptr_ = ptr;
		}
		...
		void open(const std::string & fn, const char * mode = "r")
		{
			open(fn.c_str(), mode);
		}
		...
		void swap(file_ptr & fp) noexcept
		{
			std::swap(m_ptr_, fp.m_ptr_);
		}


		//! @brief reset and reopen new file
		void reset(const std::string & fn, const char * mode = "r")
		{
			reset(fn.c_str(), mode);
		}
		...
		void write(const std::vector<char> & vec) const
		{
			std::fwrite(vec.data(), vec.size(), 1, m_ptr_);
		}

		//! @brief write into the file from c array
		void write(const void * buf, size_t size) const
		{
			std::fwrite(buf, size, 1, m_ptr_);
		}


		//! @brief write into the file from string
		void string_write(const std::string & str) const
		{
			std::fwrite(str.data(), str.size(), 1, m_ptr_);
		}

		//! @brief write into the file from zstring
		void string_write(const char * str) const
		{
			std::fwrite(str, std::strlen(str), 1, m_ptr_);
		}
