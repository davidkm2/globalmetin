...
PyObject* netSendMessengerBlockRemoveByVIDPacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	rns.SendMessengerBlockRemovePacket(rPlayer.GetName(), pInstance->GetNameString());
...
PyObject* netSendRefinePacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	if (!PyTuple_GetInteger(poArgs, 1, &iType))
		return Py_BuildException();
	int iWindowType;
	if (!PyTuple_GetInteger(poArgs, 2, &iWindowType))
		return Py_BuildException();

	CPythonNetworkStream& rns=CPythonNetworkStream::Instance();
	rns.SendRefinePacket(iSlotIndex, iType, iWindowType);	
...
{
	int sCheque;
	if (!PyTuple_GetInteger(poArgs, 0, &sCheque))
		return Py_BuildException();

	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendExchangeChequeAddPacket(sCheque);
	return Py_BuildNone();
}
#endif
...
PyObject* netSendGemShopBuy(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendGemShopBuy(bPos);
	return Py_BuildNone();
}
...
{
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendRequestRefresh();
	return Py_BuildNone();
}
...
PyObject* netSendExtendInvenButtonClick(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendExtendInvenButtonClick(bIndex);
	return Py_BuildNone();
}
...
#ifdef ENABLE_CONTACT_SYSTEM
PyObject* netSendRequestContactProfilePacket(PyObject* poSelf, PyObject* poArgs)
{
	char* szTargetName;
	if (!PyTuple_GetString(poArgs, 0, &szTargetName))
		return Py_BuildException();
	int iState;
	if (!PyTuple_GetInteger(poArgs, 1, &iState))
		return Py_BuildException();
	...
	CPythonNetworkStream & rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendContactProfileManager(szTargetName, iState, szText);
	return Py_BuildNone();
}
#endif
...
{
	PyObject* poList = PyTuple_GetItem(poArgs, 0);

	if (!PyList_Check(poList))
		return Py_BuildException();

	int iMod;
	if (!PyTuple_GetInteger(poArgs, 1, &iMod))
		return Py_BuildException();

	int List[10];
	if (iMod == 2)
		for (int i = 0; i < 10; ++i)
			List[i] = _PyInt_AsInt(PyList_GetItem(poList, i));

	CPythonNetworkStream& rkNetStream = CPythonNetworkStream::Instance();
	rkNetStream.SendSkillBookCombinationPacket(List, iMod);
...
{
	char* szToName;
	if (!PyTuple_GetString(poArgs, 0, &szToName))
		return Py_BuildException();

	CPythonNetworkStream::Instance().SendPostWriteConfirm(szToName);
	return Py_BuildNone();
}
...
PyObject* netSendPostWrite(PyObject* poSelf, PyObject* poArgs)
{
	...
	char* szTitle;
	if (!PyTuple_GetString(poArgs, 1, &szTitle))
		return Py_BuildException();

	char* szMessage;
	if (!PyTuple_GetString(poArgs, 2, &szMessage))
		return Py_BuildException();
	...
	int dwItemPos;
	if (!PyTuple_GetInteger(poArgs, 4, &dwItemPos))
		return Py_BuildException();

	int dwMoney;
	if (!PyTuple_GetInteger(poArgs, 5, &dwMoney))
		return Py_BuildException();

	int dwCheque;
	if (!PyTuple_GetInteger(poArgs, 6, &dwCheque))
		return Py_BuildException();

	CPythonNetworkStream::Instance().SendPostWrite(szToName, szTitle, szMessage, bWindowType, dwItemPos, dwMoney, dwCheque);
	return Py_BuildNone();
}
...
{
	BYTE bDataIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &bDataIndex))
		return Py_BuildException();

	CPythonNetworkStream::Instance().SendPostGetItems(bDataIndex);
	return Py_BuildNone();
}
...
{
	BYTE bButtonIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &bButtonIndex))
		return Py_BuildException();

	BYTE bDataIndex;
	if (!PyTuple_GetInteger(poArgs, 1, &bDataIndex))
		return Py_BuildException();

	CPythonNetworkStream::Instance().RequestPostAddData(bButtonIndex, bDataIndex);
	return Py_BuildNone();
}
...
PyObject* netSendPostDelete(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonNetworkStream::Instance().SendPostDelete(bDataIndex);
	return Py_BuildNone();
}
...
PyObject* netSendAttr67Add(PyObject* poSelf, PyObject* poArgs)
{
	BYTE bWindowType;
	if (!PyTuple_GetInteger(poArgs, 0, &bWindowType))
		return Py_BuildException();
	...
	BYTE bMaterialCount;
	if (!PyTuple_GetInteger(poArgs, 2, &bMaterialCount))
		return Py_BuildException();

	BYTE bSupportCount;
	if (!PyTuple_GetInteger(poArgs, 3, &bSupportCount))
		return Py_BuildException();

	BYTE bSupportWindowType;
	if (!PyTuple_GetInteger(poArgs, 4, &bSupportWindowType))
		return Py_BuildException();

	WORD wSupportSlotIndex;
	if (!PyTuple_GetInteger(poArgs, 5, &wSupportSlotIndex))
		return Py_BuildException();

	CPythonNetworkStream::Instance().SendAttr67Add(bWindowType, wSlotIndex, bMaterialCount, bSupportCount, bSupportWindowType, wSupportSlotIndex);
	return Py_BuildNone();
}
#endif
...
#endif
#ifdef ENABLE_CONTACT_SYSTEM
		{ "SendRequestContactProfilePacket",		netSendRequestContactProfilePacket,			METH_VARARGS },
#endif
#ifdef ENABLE_SHOW_CHEST_DROP
...
#endif
#ifdef ENABLE_ATTR_6TH_7TH
		{ "SendAttr67Add",							netSendAttr67Add,							METH_VARARGS },
#endif
		{ NULL,										NULL,										NULL },
	};

	PyObject* poModule = Py_InitModule(CPythonDynamicModule::Instance().GetModule(PYTHON_NETWORK).c_str(), s_methods);
...
#ifdef ENABLE_MONSTER_CARD
	PyModule_AddIntConstant(poModule, "INIT_MISSION", 0);
	PyModule_AddIntConstant(poModule, "REQUEST_MISSION", 1);
	PyModule_AddIntConstant(poModule, "RECIVE_MISSION", 2);
	PyModule_AddIntConstant(poModule, "SHUFFLE_MISSION", 3);
	PyModule_AddIntConstant(poModule, "REWARD_MISSION", 3);
	
	PyModule_AddIntConstant(poModule, "REQUEST_ILLUSTRATION", 0);
	PyModule_AddIntConstant(poModule, "MC_PROMOTION", 0);
	PyModule_AddIntConstant(poModule, "MC_TRADE", 1);
	PyModule_AddIntConstant(poModule, "MC_POLY", 2);
	PyModule_AddIntConstant(poModule, "MC_WARP", 3);
#endif
}
