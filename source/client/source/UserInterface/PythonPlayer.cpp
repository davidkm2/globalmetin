...
const TItemData * CPythonPlayer::GetItemData(TItemPos Cell) const
{
	...
	case UPGRADE_INVENTORY:
		return &m_playerStatus.aUItem[Cell.cell];
	case BOOK_INVENTORY:
		return &m_playerStatus.aBItem[Cell.cell];
	case STONE_INVENTORY:
		return &m_playerStatus.aSItem[Cell.cell];
	case ATTR_INVENTORY:
		return &m_playerStatus.aAItem[Cell.cell];
	case GIFTBOX_INVENTORY:
		return &m_playerStatus.aGItem[Cell.cell];
#endif
	default:
		return NULL;
	}
...
#ifdef ENABLE_NEW_STORAGE_SYSTEM
	case UPGRADE_INVENTORY:
		m_playerStatus.aUItem[Cell.cell] = c_rkItemInst;
		break;
	case BOOK_INVENTORY:
		m_playerStatus.aBItem[Cell.cell] = c_rkItemInst;
		break;
	case STONE_INVENTORY:
		m_playerStatus.aSItem[Cell.cell] = c_rkItemInst;
		break;
	case ATTR_INVENTORY:
		m_playerStatus.aAItem[Cell.cell] = c_rkItemInst;
		break;
	case GIFTBOX_INVENTORY:
		m_playerStatus.aGItem[Cell.cell] = c_rkItemInst;
		break;
#endif
	}
}
...
WORD CPythonPlayer::GetItemPosByVnum(BYTE window_type, DWORD dwVnum, int iStep)
{
	int s = 0;
	switch (window_type)
	{
...
#ifdef ENABLE_NEW_STORAGE_SYSTEM
		case UPGRADE_INVENTORY:
			for (int i = 0; i < c_Storage_Count; ++i)
			{
				const TItemData & c_rItemData = m_playerStatus.aUItem[i];
				if (c_rItemData.vnum == dwVnum)
				{
					if (s < iStep)
						s++;
		...
		case BOOK_INVENTORY:
			for (int i = 0; i < c_Storage_Count; ++i)
			{
				const TItemData & c_rItemData = m_playerStatus.aBItem[i];
				if (c_rItemData.vnum == dwVnum)
				{
					if (s < iStep)
						s++;
		...
		case STONE_INVENTORY:
			for (int i = 0; i < c_Storage_Count; ++i)
			{
				const TItemData & c_rItemData = m_playerStatus.aSItem[i];
				if (c_rItemData.vnum == dwVnum)
				{
					if (s < iStep)
						s++;
		...
		case ATTR_INVENTORY:
			for (int i = 0; i < c_Storage_Count; ++i)
			{
				const TItemData & c_rItemData = m_playerStatus.aAItem[i];
				if (c_rItemData.vnum == dwVnum)
				{
					if (s < iStep)
						s++;
		...
		case GIFTBOX_INVENTORY:
			for (int i = 0; i < c_Storage_Count; ++i)
			{
				const TItemData & c_rItemData = m_playerStatus.aGItem[i];
				if (c_rItemData.vnum == dwVnum)
				{
					if (s < iStep)
						s++;
...
void CPythonPlayer::GetItemAttribute(TItemPos Cell, DWORD dwAttrSlotIndex, BYTE * pbyType, int * psValue)
{
	*pbyType = 0;
	*psValue = 0;
...
void CPythonPlayer::SetItemAttribute(TItemPos Cell, DWORD dwAttrIndex, BYTE byType, int sValue)
{
	if (!Cell.IsValidCell())
		return;
...
#ifdef ENABLE_NEW_STORAGE_SYSTEM
bool CPythonPlayer::IsUpgradeInventorySlot(TItemPos Cell)
{
	return Cell.IsUpgradeInventoryCell();
}

bool CPythonPlayer::IsBookInventorySlot(TItemPos Cell)
{
	return Cell.IsBookInventoryCell();
}
...
bool CPythonPlayer::IsAttrInventorySlot(TItemPos Cell)
{
	return Cell.IsAttrInventoryCell();
}
...
void CPythonPlayer::SetSkillLevel_(DWORD dwSkillIndex, DWORD dwSkillGrade, DWORD dwSkillLevel)
{
	...
	if (pMainInstance)
	{
		if(pMainInstance->IsBattleFieldMap())
		{
			CPythonSkill::TSkillData * pSkillData;
			if (CPythonSkill::Instance().GetSkillData(dwSkillIndex, &pSkillData))
			{
				if (CPythonSkill::SKILL_TYPE_ACTIVE == pSkillData->byType && dwSkillGrade > 0)
				{
					dwSkillGrade = 3;
					dwSkillLevel = 40;
				}
...
void CPythonPlayer::Clear()
{
	...
	m_GemShopItems.clear();
	m_isGemShopWindowOpen = 0;
	m_GemShopOpenSlotCount = 0;
#endif
#ifdef ENABLE_AUTO_SYSTEM
	memset(&m_AutoSlotCoolTime, 0, sizeof(m_AutoSlotCoolTime));
	memset(&m_AutoSlotIndex, 0, sizeof(m_AutoSlotIndex));
#endif
...
#ifdef ENABLE_GEM_SYSTEM
void CPythonPlayer::SetGemShopItemData(BYTE slotIndex, const TGemShopItem & c_rItemInstance)
{
	m_GemShopItems.insert(make_pair(slotIndex, c_rItemInstance));
}

bool CPythonPlayer::GetGemShopItemData(BYTE slotIndex, const TGemShopItem ** c_ppGemShopItemData)
{
	std::map<BYTE, TGemShopItem>::iterator itor = m_GemShopItems.find(slotIndex);

	if (m_GemShopItems.end() == itor)
		return false;

	*c_ppGemShopItemData = &(itor->second);
...
#ifdef ENABLE_AUTO_SYSTEM
int CPythonPlayer::CheckSkillSlotCoolTime(BYTE bIndex, int iSlotIndex, int iCoolTime)
{

	return iCoolTime;
}

int CPythonPlayer::CheckPositionSlotCoolTime(BYTE bIndex, int iSlotIndex, int iCoolTime)
{
	return iCoolTime;
}

int CPythonPlayer::GetAutoSlotCoolTime(BYTE bIndex)
{
	if (bIndex < AUTO_SKILL_SLOT_MAX)
	{
		DWORD dwSkillSlotIndex = m_AutoSlotIndex[bIndex];
		...
		{
			TraceError("CPythonPlayer::GetAutoSlotCoolTime(bIndex=%d) - NOT CHECK", dwSkillSlotIndex);
			return 0;
		}

		CPythonSkill::TSkillData& rkSkillData=*pkSkillData;

		float Cool = rkSkillData.GetSkillCoolTime(rkSkillInst.fcurEfficientPercentage);
		...
		return m_AutoSlotCoolTime[bIndex];
}
...
void CPythonPlayer::ClearAutoSKillSlot()
{
	...
	{
		m_AutoSlotCoolTime[i] = 0;
		m_AutoSlotIndex[i] = 0;
	}
}

void CPythonPlayer::ClearAutoPositionSlot()
{
	for (int i = AUTO_SKILL_SLOT_MAX; i < AUTO_POSITINO_SLOT_MAX; ++i)
	{
		m_AutoSlotCoolTime[i] = 0;
		m_AutoSlotIndex[i] = 256;
	}
}
...
#ifdef ENABLE_BATTLE_FIELD
bool CPythonPlayer::IsBattleFieldOpen()
{
	return m_IsBattleFieldOpen;
}

void CPythonPlayer::SetBattleFieldOpen(bool bOpen)
{
	m_IsBattleFieldOpen = bOpen;
}

bool CPythonPlayer::IsBattleFieldEventOpen()
{
	return m_IsBattleFieldEventOpen;
}

void CPythonPlayer::SetBattleFieldEventOpen(bool bOpen)
{
	m_IsBattleFieldEventOpen = bOpen;
}

bool CPythonPlayer::IsBattleButtonFlush()
{
	return m_IsBattleButtonFlush;
}

void CPythonPlayer::SetBattleButtonFlush(bool bFlash)
{
	m_IsBattleButtonFlush = bFlash;
}
#endif

void CPythonPlayer::SetBasicItem(TItemPos Cell, bool bBasic)
{
	const TItemData* pData = GetItemData(Cell);
	if (!pData)
	{
	...
	(const_cast <TItemData *>(pData))->bIsBasic = bBasic;
}
...
bool CPythonPlayer::LoadMonsterCard(const char* c_szFileName)
{
	...
	{
		m_bIsIllustrationFileLoad = false;
		m_bIsIllustrationDataLoad = false;
		return false;
	}
	else
		m_bIsIllustrationFileLoad = true;
	...
	for (DWORD i = 0; i < kTextFileLoader.GetLineCount(); ++i)
	{
		...
		int iGroup = atoi(kTokenVector[MONSTER_CARD_GROUP].c_str());
		int iIndex = atoi(kTokenVector[MONSTER_CARD_INDEX].c_str());
		int iVnum = atoi(kTokenVector[MONSTER_CARD_VNUM].c_str());
		int iType = atoi(kTokenVector[MONSTER_CARD_TYPE].c_str());
		int iMap1 = atoi(kTokenVector[MONSTER_CARD_MAP1].c_str());
		int iMap2 = atoi(kTokenVector[MONSTER_CARD_MAP2].c_str());
		int iMap3 = atoi(kTokenVector[MONSTER_CARD_MAP3].c_str());
		
		TMonsterCardTable * pTable = new TMonsterCardTable;
		pTable->iGroup = iGroup;
		pTable->iIndex = iIndex;
		pTable->iVnum = iVnum;
		pTable->iType = iType;
		pTable->iMap1 = iMap1;
		pTable->iMap2 = iMap2;
		pTable->iMap3 = iMap3;
		if (m_MonsterCardDataMap.find(iGroup) == m_MonsterCardDataMap.end())
		{
			TMonsterCardGroupMap GroupMap;
			m_MonsterCardDataMap.insert(TMonsterCardDataMap::value_type(iGroup, GroupMap));
		}

		m_MonsterCardDataMap.at(iGroup).insert(TMonsterCardGroupMap::value_type(iIndex, pTable));

		if (pTable->iType == 0)
			m_iIllustrationSoloPageMax++;

		if (pTable->iType == 1)
			m_iIllustrationPartyPageMax++;
	}
	
	if (m_MonsterCardDataMap.size())
		m_bIsIllustrationDataLoad = true;
	else
		m_bIsIllustrationDataLoad = false;
