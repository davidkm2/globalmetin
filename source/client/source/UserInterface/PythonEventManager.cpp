...
void CPythonEventManager::SetTotalLineCount(TEventSet * pEventSet, std::string strEventString)
{
	...
	int a = 0;
	for (size_t i = 0, tfind; (tfind = strEventString.find(ads, i)) != string::npos; i = tfind + 1)
		a++;
...
bool CPythonEventManager::ProcessEventSet(TEventSet * pEventSet)
{
	...
	switch (iEventType)
	{
		...
		{
			PyCallClassMemberFunc(m_poInterface, "BINARY_RefreshSelectItemWindowEx", Py_BuildValue("()"));
			break;
		}
#endif
	}
...
void CPythonEventManager::MakeQuestion(TEventSet * pEventSet, script::TArgList & rArgumentList)
{
	...
	PyCallClassMemberFunc(pEventSet->poEventHandler, "MakeQuestion", Py_BuildValue("(i)", (int)rArgumentList.size()));
