...
bool CPythonNetworkStream::SendSafeBoxCheckinPacket(TItemPos InventoryPos, DWORD dwSafeBoxPos)
{
	...
	kSafeboxCheckin.dwSafePos = dwSafeBoxPos;
...
bool CPythonNetworkStream::SendSafeBoxCheckoutPacket(DWORD dwSafeBoxPos, TItemPos InventoryPos)
{
	__PlaySafeBoxItemDropSound(dwSafeBoxPos);

	TPacketCGSafeboxCheckout kSafeboxCheckout;
	kSafeboxCheckout.bHeader = HEADER_CG_SAFEBOX_CHECKOUT;
	kSafeboxCheckout.dwSafePos = dwSafeBoxPos;
...
bool CPythonNetworkStream::SendSafeBoxItemMovePacket(DWORD dwSourcePos, DWORD dwTargetPos, DWORD dwCount)
{
	__PlaySafeBoxItemDropSound(dwSourcePos);

	TPacketCGItemMove kItemMove;
	kItemMove.header = HEADER_CG_SAFEBOX_ITEM_MOVE;
	kItemMove.pos = TItemPos(INVENTORY, dwSourcePos);
	kItemMove.num = dwCount;
	kItemMove.change_pos = TItemPos(INVENTORY, dwTargetPos);
...
bool CPythonNetworkStream::RecvSafeBoxSetPacket()
{
	...
	kItemData.bIsBasic = kItemSet.bIsBasic;
...
bool CPythonNetworkStream::RecvItemSetPacket()
{
	...
	kItemData.bIsBasic = packet_item_set.bIsBasic;
...
#endif
	rkPlayer.SetBasicItem(packet_item_update.Cell, packet_item_update.bIsBasic);
