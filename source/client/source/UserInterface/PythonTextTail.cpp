...
DWORD dwVirtualID, const D3DXCOLOR & c_rColor, float fAddHeight)
{
...
#endif
#ifdef ENABLE_BATTLE_FIELD
	pTextTail->pRankingInstance=NULL;
#endif
#ifdef ENABLE_MULTI_LANGUAGE
	...
	if (!prRanking)
	{
		if(pCharacterInstance->IsPC() && pCharacterInstance->GetBattleFieldRank())
		{
			char szFileName[256];
			sprintf(szFileName, "d:/ymir work/effect/etc/ranking_battle/ranker_%d.tga", pCharacterInstance->GetBattleFieldRank());
			...
			{
				CGraphicImage * pRankingImage = (CGraphicImage *)CResourceManager::Instance().GetResourcePointer(szFileName);
				if (pRankingImage)
				{
	...
	if (strlen(c_szName) > 0)
	{
		...
		if (pInstance)
		{
			if (!strcmp(pInstance->GetNameString(), c_szName))
				...
				pTextTail->pOwnerTextInstance->SetColor(1.0f, 0.67f, 0.0f, 1.0f);
		}
