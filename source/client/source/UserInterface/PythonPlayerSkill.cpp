...
BOOL CPythonPlayer::IsUsingSkill()
{
	...
	return pkInstMain->IsUsingSkill();
}
...
{
	CItemData * pItemData;
	if (CItemManager::Instance().GetItemDataPointer(GetItemIndex(TItemPos(INVENTORY, EQUIPMENT_ARROW)), &pItemData))
...
bool CPythonPlayer::__HasItem(DWORD dwItemID)
{
	...
	for (int j = 0; j < c_Storage_Count; ++j)
	{
		...
		if (dwItemID == GetItemIndex(TItemPos(ATTR_INVENTORY, j)))
			return true;
		if (dwItemID == GetItemIndex(TItemPos(GIFTBOX_INVENTORY, j)))
			return true;
	}
#endif
	return false;
