...
{
	if (!mChildren) return true; 
	if (HasSpherePackFlag(SPF_ROOTNODE)) return false; 
...
void SpherePackFactory::Remove(SpherePack*pack)
{
	...
	if (pack->HasSpherePackFlag(SPF_ROOTNODE)) return; 
...
void SpherePack::VisibilityTest(const Frustum &f,SpherePackCallback *callback,ViewState state)
{
	...
	if (HasSpherePackFlag(SPF_SUPERSPHERE))
	{
		if (state == VS_OUTSIDE)
		{
			if (HasSpherePackFlag(SPF_HIDDEN)) return; 
		...
		else
		{
			if (state == VS_INSIDE)
			{
				if (HasSpherePackFlag(SPF_INSIDE)) return; 
