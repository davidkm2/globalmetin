...
bool Set_Proto_Mob_Table(CPythonNonPlayer::TMobTable *mobTable, cCsvTable &csvTable,
#ifdef ENABLE_EDIT_PROTO
cCsvTable &csvTable2,
#endif
...
#ifdef ENABLE_EDIT_PROTO
	col += 3;
	mobTable->dwGoldMin = atoi(csvTable2.AsStringByIndex(col2++));
	mobTable->dwGoldMax = atoi(csvTable2.AsStringByIndex(col2++));
	mobTable->dwExp = atoi(csvTable2.AsStringByIndex(col2++));
#else
...
#ifdef ENABLE_EDIT_PROTO
	col += 1;
	mobTable->dwResurrectionVnum = atoi(csvTable2.AsStringByIndex(col2++));
#else
...
#ifdef ENABLE_EDIT_PROTO
	col += 1;
	mobTable->dwPolymorphItemVnum	= atoi(csvTable2.AsStringByIndex(col2++));
#else
...
#ifdef ENABLE_EDIT_PROTO
		col += 2;
		mobTable->Skills[i].bLevel = atoi(csvTable2.AsStringByIndex(col2++));
		mobTable->Skills[i].dwVnum = atoi(csvTable2.AsStringByIndex(col2++));
#else
...
#ifdef ENABLE_EDIT_PROTO
	col += 5;
	mobTable->bBerserkPoint	= atoi(csvTable2.AsStringByIndex(col2++));
	mobTable->bStoneSkinPoint	= atoi(csvTable2.AsStringByIndex(col2++));
	mobTable->bGodSpeedPoint	= atoi(csvTable2.AsStringByIndex(col2++));
	mobTable->bDeathBlowPoint	= atoi(csvTable2.AsStringByIndex(col2++));
	mobTable->bRevivePoint	= atoi(csvTable2.AsStringByIndex(col2++));
#else
...
bool BuildMobTable()
{
	...
	if (!nameData2.Load("mob_names2.txt",'\t'))
		fprintf(stderr, "mob_names2.txt Unable to read file\n");
	else
	{
		nameData2.Next();
	...
	{
		fprintf(stderr, "mob_proto2.txt Unable to read file\n");
		return false;
	}
	data2.Next();
	int addNumber2 = 0;
	while(data2.Next())
	{
		int vnum = atoi(data2.AsStringByIndex(0));
...
#ifdef ENABLE_EDIT_PROTO
	m_iMobTableSize += data2.m_File.GetRowCount()-1 + addNumber2;
#endif
	...
	{
		fprintf(stderr, "mob_proto_edit.txt Unable to read file\n");
		return false;
	}
	data3.Next();
...
localMap))
	...
	while (data2.Next())
	{
		...
		int col = 0;
		std::map<DWORD, CPythonNonPlayer::TMobTable *>::iterator it_map_mobTable;
		it_map_mobTable = test_map_mobTableByVnum.find(atoi(data2.AsStringByIndex(col)));
		if (it_map_mobTable == test_map_mobTableByVnum.end())
...
bool BuildItemTable()
{
	...
	if (!nameData2.Load("item_names2.txt",'\t'))
		fprintf(stderr, "item_names2.txt Unable to read file\n");
	else
	{
		nameData2.Next();
		while(nameData2.Next())
	...
	{
		fprintf(stderr, "item_proto2.txt Unable to read file\n");
		return false;
	}
	data2.Next();
...
#ifdef ENABLE_EDIT_PROTO
	m_iItemTableSize += data2.m_File.GetRowCount()-1+addNumber2;
#endif
	...
	while (data2.Next())
	{
		...
		std::map<DWORD, CItemData::TItemTable *>::iterator it_map_itemTable;
		it_map_itemTable = test_map_itemTableByVnum.find(atoi(data2.AsStringByIndex(col)));
...
int main(void)
{
	...
	printf("Lutfen Yapmak ?stedi?iniz ??lemi Seciniz:\n");
	printf("Acma : 0\n");
	printf("Kapatma : 1\n");
	...
	{
		case 0:
			printf("Lutfen Acmak ?stedi?iniz Protoyu Seciniz:\n");
			printf("?tem : 0\n");
			printf("Mob : 1\n");
			printf("Her ?kisi : 2\n");
			bas1:
			...
			switch(b)
			{
				...
				case 2:
					...
					break;
				default:
					printf("Lutfen Gecerli ??lemi Seciniz !!");
					goto bas1;
			}
			break;
		case 1:
			printf("Lutfen Kapatmak ?stedi?iniz Protoyu Seciniz:\n");
			printf("?tem : 0\n");
			printf("Mob : 1\n");
			printf("Her ?kisi : 2\n");
			bas2:
